...
def create_dataset(dataset, config):
    ...
    normalize = transforms.Normalize((0.2344, 0.2344, 0.2344), (0.1946, 0.1946, 0.1946))
    
    pretrain_transform = transforms.Compose([    
        # transforms.Pad(padding=[0, 128, 0, 128], padding_mode='constant') ,                  
            # transforms.RandomResizedCrop(config['image_res'], scale=(0.8, 1.0), interpolation=Image.BICUBIC),
            # transforms.RandomHorizontalFlip(), # Aisha: horizontal flip will lose the notion of left and right for our case
            transforms.Resize(config['image_res'], interpolation=Image.BICUBIC),
            RandomAugment(2,7,isPIL=True,augs=['Identity','AutoContrast','Brightness','Sharpness',
    ...
    if dataset=='pretrain':
        dataset = pretrain_dataset(config['train_file'], pretrain_transform, max_words=config['max_words'])                  
        return dataset      
        ...
        max_words=config['max_words'],  num_samples=config['num_samples'])  
