...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i,(image, text, idx) in tqdm(enumerate(metric_logger.log_every(data_loader, print_freq, header))):
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    num_text = len(texts)
    print(num_text, data_loader.dataset.__len__())
    text_bs = 256
    text_feats = []
    text_embeds = []  
    text_atts = []
...
def main(args, config):
    ...
    for epoch in range(0, max_epoch):
        ...
        if config['selective_sampling']:
            ...
            train_loader.dataset.shuffle(bs=config['batch_size_train'],
                                        rare_grp_ratio=config['rare_grp_ratio'],
                                        batch_shuffle=config['batch_shuffle']
                                        )

        # elif (config['selective_sampling'] and train_loader.dataset.shuffle) or (not config['selective_sampling'] and not train_loader.dataset.shuffle):
        #     print("Incorrect combination of parameters: selective_sampling and shuffle cannot be\
        #         true/false at the same time, may lead to erroneous behavior.")
