    ...
    def evaluate(self, eval_dataloader=None):
        ...
        if self.mode == 'multilabel':
            ...
            for i in range(pred_score.shape[1]):
                ...
                if sum(y_cls) > 0: #to deal with classes with no true values in test data
                    auprc_list.append(average_precision_score(y_cls, pred_cls))
                    auroc_list.append(roc_auc_score(y_cls, pred_cls)) #commented out by AU
            outputs['auc'] = np.mean(auroc_list)
            outputs['auprc'] = np.mean(auprc_list) #commented out by AU
        return outputs
