...
from .prompts import generate_chexpert_class_prompts, generate_uwmadison_class_prompts
...
class ImageTextContrastiveDataset(Dataset):
    ...
    _labels_ = ['No Finding', 'mass', 'features', 'calcification', 'asymmetry', 'surgical change', 'distortion', 'fatty', 'scattered fibroglandular density',
          'heterogeneously dense', 'extreme dense', 'lesion', 'implant']
    def __init__(self, datalist=['uwmadison-train'], imgtransform=None) -> None:
        ...
        if imgtransform is None:
            self.transform = transforms.Compose([
                ...
                transforms.Normalize(mean=[0.2344],std=[0.1946])],
            )
        else:
    ...
    def __getitem__(self, index):
        ...
        # print(f"ITCD: img before transform:{img.size}")
        # img = self._pad_img(img) # pad image to square
        try:
            img = self.transform(img).unsqueeze(0)
        except:
            print(row.imgpath)
            img = torch.zeros(1, 3, constants.IMG_SIZE, constants.IMG_SIZE)
        # print(f"ITCD: img after transform:{img.shape}")
        report = row.report # original sentences list
        img_label = row[self._labels_].values # image corresponds to text labels
        text_label = row[self._labels_].values
        ########### AU: commented out because text labels same as img lbels for us ###########
        ...
        #     # sent_ix = random.randint(0, len(report)-1) #uncomment if sent-image training, turned off for uw-mad
    ...
    def _build_prompt_sentence(self, n = 200):
        ...
        print(sentence_label.keys())
        new_sent_list = []
        for task in constants.UWMADISON_TASKS:
    ...
    def shuffle(self, bs=8, rare_grp_ratio=0.375, batch_shuffle=False):
        ...
        #converting to dataframe because other methods are operating on dataframe instead of list of items
        self.df = pd.DataFrame(self.ann)
...
class ImageTextContrastiveCollator:
    ...
    def __call__(self, batch):
        ...
        # print([inp.shape for inp in inputs['text_labels']])
...
class ZeroShotImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        row = self.df.iloc[index]
        img = Image.open(row.imgpath)
        # print(f"ZSID: img before transform:{img.size}")
        # img = self._pad_img(img)
        # print(f"ZSID: img after padding:{img.size}")
        img = self.transform(img).unsqueeze(0)
        # print(f"ZSID: img after transform:{img.shape}")
